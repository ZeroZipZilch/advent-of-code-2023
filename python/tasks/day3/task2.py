...
def run(input_rows: list[str]):
  ...
  for adjecent_number in adjecent_numbers:
    if adjecent_number.row_number == -1:
...
def check_adjencent_for_numbers(symbol: NumberCoordinates, numbers: list[NumberCoordinates], adjecent_numbers: list[NumberCoordinates]):
    ...
    for number in numbers:
      ...
      if adjecent_horizontal and adjecent_vertical:
        ...
        if symbol.value == '*':
          multiply_numbers.append(adjecent_number)
          delete_numbers.append(adjecent_number)

        if not object_exists_in_list(adjecent_number, adjecent_numbers):
          adjecent_numbers.append(adjecent_number)

    if symbol.value == '*' and len(multiply_numbers) == 2:
      # print("multiply_numbers")
      # for number in multiply_numbers:
      #   print(number.row_number, number.start_x, number.end_x, number.value)

      # Multiply numbers into adjecent_numbers
      multiplied_number = NumberCoordinates()

      multiplied_number.value = multiply_numbers[0].value * multiply_numbers[1].value
      multiplied_number.start_x = -1
      multiplied_number.end_x = -1
      multiplied_number.row_number = -1
      
      if not object_exists_in_list(multiplied_number, adjecent_numbers):
        adjecent_numbers.append(multiplied_number)
